...
update_kubeconfig() {
    IP=$1
    if [ -z "$2" ]; then
        IP_FOR_KUBE_CONFIG=${IP}
    else    
        IP_FOR_KUBE_CONFIG=${2}
    fi  
    CURRENT_SERVER=$(cat ${VKS_HOME}/admin.conf | grep server: | awk '{print $2}')
    sed -i.bak "s|${CURRENT_SERVER}|https://${IP_FOR_KUBE_CONFIG}:${KIND_CLUSTER_NODEPORT}|g" ${VKS_HOME}/admin.conf
...
update_kubeconfig $CLUSTER_IP $externalIP
